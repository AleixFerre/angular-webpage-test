...
export class ContactsService {
  ...
  addContact(contact: Contact) {
    this.http.post<{ id: string }>('http://localhost:3000/contacts', contact).subscribe(responseData => {
  ...
  updateContact(id: string, contact: Contact) {
    this.http.put(`http://localhost:3000/contacts/${id}`, contact).subscribe(() => {
  ...
  deleteContact(contactId: string) {
    this.http.delete(`http://localhost:3000/contacts/${contactId}`).subscribe(() => {
